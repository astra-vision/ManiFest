...
class BaseModel(pl.LightningModule):
    ...
    def log_current_losses(self):
        ...
        for name in self.loss_names:
            if isinstance(name, str):
                ...
                self.logger.experiment.log({'loss_{}'.format(name): loss_value}, self.trainer.global_step)
    ...
    def log_current_visuals(self):
        ...
        for key, viz in visuals.items():
            img = ToPILImage()(viz[0].cpu())
            self.logger.experiment.log({'img_{}'.format(key): wandb.Image(img)}, self.trainer.global_step)
